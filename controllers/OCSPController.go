...
func HandleOCSP(ctx fiber.Ctx) error {
	...
	var exists bool
	db.Get(&exists, "SELECT EXISTS(SELECT 1 FROM certs WHERE serial_number = ?)", serialNumber)
	log.Printf("Найден сертификат в БД certs с серийным номером %s: %v", serialNumber, exists)

	// Для надежности проверим также регистр-независимый поиск
	var existsCI bool
	db.Get(&existsCI, "SELECT EXISTS(SELECT 1 FROM certs WHERE LOWER(serial_number) = LOWER(?))", serialNumber)
	if existsCI && !exists {
		...
		db.Get(&correctSerial, "SELECT serial_number FROM certs WHERE LOWER(serial_number) = LOWER(?)", serialNumber)
		if correctSerial != "" {
...
func GetCertificateStatus(db *sqlx.DB, serialNumber string) (int, time.Time, int, error) {
	...
	err := db.Get(&existsInCerts, "SELECT EXISTS(SELECT 1 FROM certs WHERE serial_number = ?)", serialNumber)
	if err != nil {
		log.Printf("Ошибка при проверке существования сертификата в таблице certs: %v", err)
	} else {
		log.Printf("Существование сертификата в таблице certs: %v", existsInCerts)
	}

	// Если сертификат не найден, попробуем поиск без учета регистра
	if !existsInCerts {
		var existsCaseInsensitive bool
		err := db.Get(&existsCaseInsensitive, "SELECT EXISTS(SELECT 1 FROM certs WHERE LOWER(serial_number) = LOWER(?))", serialNumber)
		if err == nil && existsCaseInsensitive {
			...
			var correctSerial string
			err = db.Get(&correctSerial, "SELECT serial_number FROM certs WHERE LOWER(serial_number) = LOWER(?)", serialNumber)
