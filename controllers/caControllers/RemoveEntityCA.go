...
func RemoveEntityCA(c fiber.Ctx) error {
	...
	if c.Method() == "POST" {
		...
		dataRemove := `DELETE FROM entity_ca WHERE id = ?`
		_, err = tx.Exec(dataRemove, data.Id)
		if err != nil {
			tx.Rollback() // Откатываем транзакцию при ошибке
			return c.Status(500).JSON(fiber.Map{
				"status":  "error",
				"message": "Error deleting entity CA: " + err.Error(),
			})
		}
	...
	return c.Render("ca/entityCAList-tpl", fiber.Map{})
}
