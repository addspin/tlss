...
func StartCombinedCRLGeneration(updateInterval time.Duration, db *sqlx.DB) {
...
func CombinedCRL(db *sqlx.DB) error {
	...
	// Добавляем Root CA CRL как отдельный PEM блок
	bundlePem = append(bundlePem, rootCAPem...)

	// Добавляем Sub CA CRL как отдельный PEM блок
	bundlePem = append(bundlePem, subCAPem...)
	// Сохраняем бандл CRL в базу данных
	_, err = db.Exec(`
		INSERT OR REPLACE INTO crl (type_crl, data_crl) VALUES (?, ?)
