...
func GenerateCRLUser() error {
	...
	err = db.Get(&crlInfo, "SELECT * FROM crl_info_user LIMIT 1")
	if err == sql.ErrNoRows {
		...
		if err != nil {
			return fmt.Errorf("не удалось вставить информацию о user CRL: %w", err)
		}
	} else if err != nil {
		return fmt.Errorf("не удалось получить информацию о user CRL: %w", err)
	} else {
		...
		`, crlInfo.LastUpdate, crlInfo.NextUpdate, crlInfo.CrlNumber)
		if err != nil {
			return fmt.Errorf("не удалось обновить информацию о user CRL: %w", err)
		}
	}
	...
	if err != nil {
		return fmt.Errorf("не удалось сохранить user CRL: %w", err)
	}
