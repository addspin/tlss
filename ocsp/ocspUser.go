...
// UserOCSPResponder представляет OCSP-респондер
type UserOCSPResponder struct {
	db             *sqlx.DB
...
func StartUserOCSPResponder(updateInterval time.Duration) {
	...
	responder, err := NewUserOCSPResponder(db, updateInterval)
	if err != nil {
	...
	if err := responder.UpdateUserOCSPData(); err != nil {
...
func NewUserOCSPResponder(db *sqlx.DB, updateInterval time.Duration) (*UserOCSPResponder, error) {
	// Инициализируем OCSP-респондер
	responder := &UserOCSPResponder{
		db:             db,
...
func (r *UserOCSPResponder) UpdateUserOCSPData() error {
	log.Println("Обновление данных user OCSP...")
	...
	`)
	if err != nil {
		return fmt.Errorf("ошибка запроса пользовательских отозванных сертификатов: %v", err)
	}

	log.Printf("Найдено %d пользовательских отозванных сертификатов", len(certs))
	...
	for _, cert := range certs {
		...
		// Проверяем, существует ли уже запись для этого сертификата в таблице user_ocsp_revoke
		var existingId int
		err = r.db.QueryRow(`
			SELECT id FROM user_ocsp_revoke 
			WHERE serial_number = ?`, cert.SerialNumber).Scan(&existingId)

		if err == nil {
			// Если запись существует, обновляем её
			_, err = r.db.Exec(`
				UPDATE user_ocsp_revoke SET
					common_name = ?, cert_create_time = ?, cert_expire_time = ?, 
			...
			if err != nil {
				log.Printf("Ошибка обновления данных для отозванного сертификата %s (%s): %v", cert.SerialNumber, cert.CommonName, err)
				continue
			}
			log.Printf("Обновлена запись для отозванного сертификата %s (%s)", cert.SerialNumber, cert.CommonName)
		} else {
			// Если записи нет, вставляем новую в таблицу user_ocsp_revoke
			_, err = r.db.Exec(`
				INSERT INTO user_ocsp_revoke (
					common_name, cert_create_time, cert_expire_time, days_left, 
				...
				cert.CertStatus,
				r.SubCACert.Subject.String(),
				standardizeSerialNumberUser(r.SubCACert.SerialNumber),
				issuerNameHash,
				issuerKeyHash,
				hashAlgo,
				nowStr,
			...
			if err != nil {
				log.Printf("Ошибка вставки данных для отозванного сертификата %s (%s): %v", cert.SerialNumber, cert.CommonName, err)
				continue
			}
			log.Printf("Добавлена новая запись для отозванного сертификата %s (%s)", cert.SerialNumber, cert.CommonName)
		}
...
func calculateIssuerHashesUser(issuerCert *x509.Certificate) (string, string, string) {
...
func parseRevocationReasonUser(reason string) int {
	switch reason {
	case "keyCompromise":
...
func (r *UserOCSPResponder) GetSignatureAlgorithm() x509.SignatureAlgorithm {
...
func (r *UserOCSPResponder) CreateResponse(template ocsp.Response) ([]byte, error) {
...
func (r *UserOCSPResponder) GetCertificateStatus(serialNumber string) (int, time.Time, int) {
	// Ищем сначала в кеше
	r.mu.RLock()
...
func standardizeSerialNumberUser(serialNumber *big.Int) string {
